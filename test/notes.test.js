      ...
      return Promise.all([
        ...
        .then(([data, res]) => {
          ...
          res.body.forEach(function (item, i) {
            ...
            expect(item).to.include.all.keys('userId', 'id', 'title', 'createdAt', 'updatedAt', 'tags');
    ...
    it('should return correct search results for a title search', function () {
      ...
      return Promise.all([dbPromise, apiPromise])
        .then(([data, res]) => {
          ...
          res.body.forEach(function (item, i) {
            expect(item).to.be.a('object');
            expect(item).to.include.all.keys('userId', 'id', 'title', 'createdAt', 'updatedAt', 'tags'); // Note: folderId and content are optional
            expect(item.id).to.equal(data[i].id);
            expect(item.title).to.equal(data[i].title);
            expect(item.content).to.equal(data[i].content);
            expect(new Date(item.createdAt)).to.eql(data[i].createdAt);
    ...
    it('should return correct notes', function () {
      ...
      return Note.findOne({ userId: user.id })
        ...
        .then((res) => {
          ...
          // Note: folderId, tags and content are optional
          expect(res.body).to.include.all.keys('userId', 'id', 'title', 'createdAt', 'updatedAt');
    ...
    it('should create and return when folderId is an empty string', function () {
      ...
      return chai.request(app)
        ...
        .then(function (_res) {
          ...
          expect(res).to.be.json;
          expect(res.body).to.be.a('object');
          expect(res.body).to.include.keys('userId', 'id', 'title', 'createdAt', 'updatedAt', 'tags');
          return Note.findOne({ _id: res.body.id });
        })
        .then(data => {
          expect(res.body.id).to.equal(data.id);
    ...
    it('should return an error when `folderId` is not valid ', function () {
      ...
      return chai.request(app)
        ...
        .then(res => {
          ...
          expect(res).to.be.json;
          expect(res.body).to.be.a('object');
          expect(res.body.message).to.equal('The folderId is not valid1');
        });
    });
    ...
    it('should return an error when a tag `id` is not valid ', function () {
      ...
      return chai.request(app)
        ...
        .then(res => {
          ...
          expect(res).to.be.json;
          expect(res.body).to.be.a('object');
          expect(res.body.message).to.equal('The tags array contains an invalid id1');
        });
    });
    ...
    it('should update the note when provided a valid title', function () {
      ...
      Note.findOne({ userId: user.id })
        ...
        .then(function (res) {
          ...
          expect(res).to.be.json;
          expect(res.body).to.be.a('object');
          expect(res.body).to.include.keys('userId', 'id', 'title', 'content', 'createdAt', 'updatedAt');
          expect(res.body.id).to.equal(data.id);
    ...
    it('should update the note when provided a valid tag', function () {
      ...
      Promise.all([
        ...
        .then(([tag, note]) => {
          ...
          data = note;
          chai.request(app).put(`/api/notes/${note.id}`)
